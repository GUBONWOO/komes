...
// HOMES 페이지 CSS 셀렉터 — test-crawl.js 실행 후 실제 클래스로 수정
const SEL = {
...
const buildItems = (rawItems, urlType, areaName) =>
  rawItems
    .map((raw) => {
      ...
      return {
        ...
        building_area:     raw.buildingArea || null,
...
const scrapePage = async (page, url) => {
  ...
  if (!hasCards) return { rawItems: [], totalPages: 0 };

  return page.evaluate((SEL) => {
    const q = (el, sel) => {
      try { return el.querySelector(sel)?.textContent?.trim() || null; } catch { return null; }
    };
    const cards = Array.from(document.querySelectorAll(SEL.card));
    const rawItems = cards.map((card) => {
      const link = card.querySelector('a[href*="/mansion/b-"], a[href*="/kodate/b-"]')
...
const crawlAreaType = async (browser, area, urlType) => {
  const buildUrl = (p) => {
  ...
  try {
    ...
    const totalItems = parseInt((totalStr || '0').replace(/[^0-9]/g, ''), 10) || 0;
    ...
    for (let p = 2; p <= totalPages; p++) {
      ...
      try {
        ...
        console.warn(`  [${area.name}/${urlType.type}] ${p}p 스킵: ${err.message}`);
      }
    }
  } finally {
    await page.close();
...
const saveProperties = async (items, area, crawledTypes = null) => {
...
const runCrawler = async (areaNames = null, targetTypes = null) => {
    ...
    for (const area of areas) {
      ...
      for (const urlType of urlTypes) {
        ...
        try {
          ...
          console.error(`[${area.name}/${urlType.type}] 크롤링 오류:`, err.message);
        }
        if (items.length > 0) {
          try {
            ...
            console.error(`[${area.name}/${urlType.type}] DB 오류:`, err.message);
          }
        }
...
module.exports = { runCrawler, TARGET_LINES: TARGET_AREAS, CRAWL_GROUPS };
